"""Minimal stdio MCP client and adapter for the local tool registry."""
...
def _call(client, tool_name, args):
    result = client.call_tool(tool_name, args)
    content = result.get("content", [])
    text = "\n".join(str(item.get("text", "")) for item in content if isinstance(item, dict))
    return ToolResult(text or json.dumps(result, ensure_ascii=False), is_error=bool(result.get("isError")))
