...
REPO="${PICO_REPO:-https://github.com/Sylvia145/moka.git}"
INSTALL_DIR="${PICO_INSTALL_DIR:-$HOME/.pico-agent}"
BRANCH="${PICO_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[moka]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[moka]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[moka]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[moka] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/moka"
    LEGACY_LAUNCHER="$BIN_DIR/pico"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/moka" "\$@"
EOF
    chmod +x "$LAUNCHER"
...
EOF
    ...
    printf "  兼容入口：${CYAN}${LEGACY_LAUNCHER}${RESET}\n\n"
}
